...
'''
调取catch_keywords得到的关键字列表，作为第一部分要输出的关键字
...
import catch_keywords
import numpy as np

embed_file = 'data/word2vec_format.txt'
...
def stopwordslist():
    ...
    stopwords = [line.strip() for line in open(
        'data/stopwords.txt', encoding='UTF-8').readlines()]
    return stopwords


def seg_depart(txt_str):
    # 输入处理PDF得到的txt内容，输出切分去重后的全文的词语list-->get_words
...
def get_final_keywords(file_path, usr_keyword):
    # 输入目标PDF文件路径，tfidf算法得到的关键词，用户输入的主题词（list类型）,输出全部关键词
    input_txt = pdf_to_txt(file_path)
    get_words = seg_depart(input_txt)
    tfidf_keywords = catch_keywords.tfidf_extract(input_txt)

    usr_keywords = []
    for key_word in usr_keyword:
        # 对于用户输入的每个主题词进行全文词距离计算，得到每个词的近义词作为相关关键词，输出到相关关键词列表
        distance = {}
...
if __name__ == '__main__':
    file_path = 'test.pdf'
    embedfile_path = 'data/word2vec_format.txt'
